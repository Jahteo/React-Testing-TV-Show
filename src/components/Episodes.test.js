...
//set up Pretier extension to take care of formatting this eventually.
const episodesData = [
  {
    id: 553946,
...
test("Rerenders with new props", () => {
  ...
  //querries that season 2, episode 1 renders:
  const episodes = screen.getAllByTestId(/episodes/i);
  expect(episodes).toHaveLength(4);

  expect(screen.getByText(/A young boy mysteriously disappears/)).toBeTruthy();
})